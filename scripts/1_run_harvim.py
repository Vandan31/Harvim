...
def run_harvim_on_image(image_path, output_dir):
    ...
    if os.path.exists(f"checkpoints/watermark_cvae_64.pth"):
        cvae.load_state_dict(torch.load(f"checkpoints/watermark_cvae_64.pth", map_location=device))
    else:
        print("CVAE weights not found. Using untrained CVAE.")
    
    # We want digit '3' for example, with initial padding mostly centered (0.5, 0.5)
    class_idx = 0
    ...
    checkpoint = torch.load("/home/hem436/Documents/harvim/checkpoints/real_ckp/realnvp_epoch_28.pt", map_location=device)
    ...
    optimal_watermark = F.interpolate(optimal_watermark, size=(IMAGE_SIZE, IMAGE_SIZE), mode='bilinear', align_corners=False)
    ...
    watermarked_image = A_m * x_f + (1-A_m)* x_f * 0.45 +optimal_watermark # White watermark for simplicity
